...
calculate_happiness = function(df, nResource, nGroup, nActor){
...
simulation = function(name,nSim,nRounds,nGroup,nActor,nResource,rOffset,sdGather,gatherSuccess,ingroupP, degration, premade_nw=F, nw_df_list,inGfav){
  ...
  data = foreach(sim = 1:nSim, .combine = "rbind", 
    ...
    memory_df = data.frame(Actor = 1:(nActor*nGroup), InCoopMem = 0.5, InDefMem = 0.5, OutCoopMem = 0.5, OutDefMem = 0.5) 
    ...
    for (round in 1:nRounds){ # loop through rounds
      ...
      resource_df = calculate_happiness(resource_df,nResource,nGroup,nActor)
...
test_sim_results = simulation(name = "test_sim",
                         nSim = 1,
                         nRounds = 100, # Should not be changed above 1000 if running with network1000
