...
for (i in seq(from = 0.1, to = 1, by = 0.01)){
  sim = simulation(name = paste("prior test -",i),
                         nSim = 4,
                         nRounds = 200, # Should not be changed above 1000 if running with network1000
                         nGroup = 8, # Should not be changed if running with network1000
                         nActor = 5, # Should not be changed if running with network1000
...
test_sim_results2 = simulation(name = "test_sim",
                         nSim = 2,
                         nRounds = 100,
                         nGroup = 12, 
                         nActor = 18, 
...
for (i in seq(from = 0, to = 0.5, by = 0.1)){
...
ggplot(cost_results, aes(x = Round, y = CoopWeight)) +
  geom_point(aes(color = as.factor(cost_results$cost))) +
...
sdGather_list = c(0.25, 0.5, 0.75, 1)
...
for (p in ingroupP_list){
  for (g in sdGather_list){
    sim = simulation(paste("simulation  -  P:",p,"    G:",g),
                     nSim = 100,
